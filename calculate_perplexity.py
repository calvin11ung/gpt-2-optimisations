...
def compute_perplexity(
        ...
        for start_index in tqdm.tqdm(range(0, len(encoded_texts), batch_size)):
            ...
            with torch.no_grad():
                if isinstance(model, ctransformers.llm.LLM):
                    out_logits = []
                    for token in encoded_batch[0]:
                        model.eval([token])
                        out_logits.append(list(model.logits))
                    out_logits = torch.Tensor(out_logits).unsqueeze(0)
                else:
